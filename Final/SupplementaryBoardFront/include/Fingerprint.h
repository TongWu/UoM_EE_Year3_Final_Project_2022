		 ...
		 */
			if((fingerprint.collectionFingerprint(/*timeout=*/3)) != ERR_ID809){
				...
				while(fingerprint.detectFinger()){
					delay(50);
					j++;
					if(j == 15){             //Yellow LED blinks quickly 3 times, means it's in fingerprint regisrtation mode now
						/*Set fingerprint LED ring to always ON in yellow*/
						fingerprint.ctrlLED(/*LEDMode = */fingerprint.eFastBlink, /*LEDColor = */fingerprint.eLEDYellow, /*blinkCount = */3);
					}else if(j == 30){      //Red LED blinks quickly 3 times, means it's in fingerprint deletion mode now
						/*Set fingerprint LED ring to always ON in red*/
						fingerprint.ctrlLED(/*LEDMode = */fingerprint.eFastBlink, /*LEDColor = */fingerprint.eLEDRed, /*blinkCount = */3);
					}
				}
